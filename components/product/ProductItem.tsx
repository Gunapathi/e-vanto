...
const ProductItem = ({ data, classname, showDesc }: any) => {
	...
	return (
		<div className={`px-[10px] ${classname ? classname : ""}`}>
			<div
				className={`group mb-[30px] rounded-lg shadow-[rgba(0,0,0,0.24)_0px_0px_6px] bg-slate-100 `}>
				...
				<div className="p-4">
					...
					<div className="flex h-[30px]">
						...
						<input
							type="text"
							value={qty}
							className={`w-full h-full rounded-md mx-2 px-2 bg-white outline-none max-w-[50px] text-center`}
							onChange={(e: any) => {
								e.target.value < 0
									? setQty(0)
									: e.target.value == 0
